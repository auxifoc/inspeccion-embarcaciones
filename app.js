...
var equipo_rd339_default = {
  ...
  reglas: [
    ...
    },
    {
      ...
      advertencia: "El sistema no conoce el tipo de ancla: si no es de alto poder de agarre, el peso exigible es un tercio mayor que el que aqu\xED se indica y hay que comprobarlo a mano."
    },
    {
...
function avanceDeBloque(bloque, hallazgos) {
  const respondidos = bloque.puntos.filter((p) => hallazgos[p.codigo] !== void 0).length;
  return { respondidos, total: bloque.puntos.length };
}
...
function textoAvanceBloque(bloque, hallazgos) {
  const { respondidos, total } = avanceDeBloque(bloque, hallazgos);
  return `${respondidos}/${total}`;
}
function pintarPunto(punto, hallazgo, datos, historico, acciones, soloLectura, abierto) {
  const resultado = hallazgo?.resultado;
  const propuesta = proponerGravedad(punto.codigo);
  return h(
    "details",
    {
      class: `punto ${resultado ?? "sin-responder"}`,
      // Con qué punto se corresponde este nodo, para poder sustituirlo solo a él.
      "data-punto": punto.codigo,
    ...
    h(
      "summary",
      {
        ...
        onclick: (e) => {
          ...
          acciones.alAbrirPunto(punto.codigo, !detalle.open);
        }
      },
    ...
    h(
      ...
      (hallazgo?.fotos.length ?? 0) > 0 && h(
        ...
        { class: "miniaturas" },
        ...(hallazgo?.fotos ?? []).map(
          (clave) => miniatura(punto.codigo, clave, acciones, soloLectura)
        )
      )
    )
  );
...
function buscadorDeGuion(total) {
  ...
  const aplicar = (texto, soloPendientes2) => {
    ...
    for (const bloque of document.querySelectorAll("[data-bloque]")) {
      const alguno = [...bloque.querySelectorAll("[data-punto]")].some((p) => !p.hidden);
      bloque.hidden = !alguno;
    }
...
function pintarGuion(guion, hallazgos, datosPunto, registro, visitaActiva, acciones, soloLectura = false, puntoAbierto) {
  ...
  return h(
    ...
    ...guion.bloques.map(
      (bloque) => h(
        "section",
        { class: "bloque", "data-bloque": bloque.codigo },
        h(
          "h2",
          {},
          ...
          h("span", { class: "contador-bloque" }, textoAvanceBloque(bloque, hallazgos)),
...
function panelResultado(resultado, avance) {
  ...
  return h(
    ...
    resultado.noAccesibles.length > 0 && h(
      "p",
      { class: "salvedad" },
      `\u26A0 ${cuenta(resultado.noAccesibles.length, "punto no accesible", "puntos no accesibles")}: ` + resultado.noAccesibles.map((n) => n.puntoId).join(", ")
    ),
...
function describirConsecuencia(entonces) {
  ...
  if (typeof cantidad === "number" && entonces["minimoPorUnidad"] === true) {
    ...
    partes.push(
      `\u2014 seg\xFAn tabla por ${NOMBRE_VARIABLE[tabla.variable] ?? tabla.variable} (${extremos}, interpolando las intermedias)` + (tabla.tolerancia !== void 0 ? `, con una tolerancia del ${Math.round(tabla.tolerancia * 100)} %` : "")
    );
...
async function responder(puntoId, resultado) {
  ...
  if (resultado === "conforme" || resultado === "no_aplica") avanzarAlSiguiente(puntoId);
  await guardarInspeccion(inspeccion);
}
...
function repintarPunto(puntoId) {
  ...
  const punto = puntosDe(guion).find((p) => p.codigo === puntoId);
  if (punto === void 0) {
    void render();
    return;
  }
  ...
  const bloque = guion.bloques.find((b) => b.puntos.some((p) => p.codigo === puntoId));
  const contadorBloque = document.querySelector(
    `[data-bloque="${bloque?.codigo ?? ""}"] .contador-bloque`
  );
  if (bloque !== void 0 && contadorBloque !== null) {
    contadorBloque.textContent = textoAvanceBloque(bloque, inspeccion.hallazgos);
  }
}
